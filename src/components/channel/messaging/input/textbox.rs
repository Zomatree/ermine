use std::{borrow::Cow, sync::LazyLock, time::{Duration, SystemTime}};
...
impl Component for Textbox {
    fn render(&self) -> impl IntoElement {
        ...
        use_side_effect_with_deps(&editable.editor().read().to_string(), {let id = self.channel.read().id().to_string(); move |content| {
            let ts = last_typing.peek().cloned();
            let now = SystemTime::now();

            if (content.is_empty() && ts.is_none()) || ts.is_some_and(|ts| now.duration_since(ts).unwrap().as_secs() < 2) {
                return
            };
            ...
            events.send(ClientMessage::BeginTyping { channel: id.clone() }).unwrap();

            stop_typing.take().map(|t| t.cancel());
            stop_typing.set(Some(spawn_forever(async move {
                sleep(Duration::from_secs_f32(2.5)).await;
                events.send(ClientMessage::EndTyping { channel: id.clone() }).unwrap();
            })));
        }});
        ...
        rect()
            ...
            .child(ScrollView::new().height(Size::Inner).max_height(Size::window_percent(32.)).width(Size::flex(1.)).child(if !can_send_messages {
                rect().child(
                    label()
                        .font_size(14.)
                                ...
                                move |e: Event<KeyboardEventData>| {
                                        ...
                                        && editable.editor().read().to_string().len() == 0
                                    {
                                        ...
                                        if e.key == Key::Character("v".to_string())
                                            && e.modifiers.contains(Modifiers::ctrl_or_meta())
                                            && let Ok(paths) = clipboard.write().get().file_list()
                                            && !paths.is_empty()
                                        {
                                            spawn(async move {
                                                for path in paths {
